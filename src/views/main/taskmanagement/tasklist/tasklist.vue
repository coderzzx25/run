<template>
  <div class="tasklist">
    <main-head titleName="任务列表" btnName="添加任务" @onClickAdd="onClickAdd" />
    <div class="content">
      <main-search
...
const requestService = (searchValue?: any) => {
  taskStore.listAction({ ...pageValue, ...searchValue })
}
//数据删除
...
const increaseService = (data: any) => {
  taskStore.increaseAction(data)
}
//数据修改
const updataService = (data: any) => {
  taskStore.updataAction(data)
}
...
requestService()
const { list, count } = storeToRefs(taskStore)
