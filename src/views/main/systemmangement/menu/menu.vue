<template>
  <div class="menu">
    <main-head titleName="菜单列表" btnName="添加菜单" @onClickAdd="onClickAdd" />
    <div class="content">
      <main-search
...
const requestService = (searchValue?: any) => {
  menuStore.listAction({ ...pageValue, ...searchValue })
}
//数据删除
const deleteServie = (id: number) => {
  menuStore.deleteAction(id)
}
//数据添加
const increaseService = (data: any) => {
  menuStore.increaseAction(data)
}
//数据修改
const updataService = (data: any) => {
  menuStore.updataAction(data)
}
...
requestService()
const { list, count } = storeToRefs(menuStore)
