<template>
  <div class="schoollist">
    <main-head titleName="学校列表" btnName="添加学校" @onClickAdd="onClickAdd" />
    <div class="content">
      <main-search
...
import useSchoolStore from '@/store/school/school'

const schoolStore = useSchoolStore()
//数据请求
const requestService = (searchValue?: any) => {
  schoolStore.listAction({ ...pageValue, ...searchValue })
}
//数据删除
const deleteServie = (id: number) => {
  schoolStore.deleteAction(id)
}
//数据添加
const increaseService = (data: any) => {
  schoolStore.increaseAction(data)
}
//数据修改
const updataService = (data: any) => {
  schoolStore.updataAction(data)
}
...
requestService()
const { list, count } = storeToRefs(schoolStore)
